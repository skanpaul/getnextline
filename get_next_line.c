...
char	*get_next_line(int fd)
{
	...
	int				line_done; // 0: line NOT complete | 1: line complete 
	...
	while (line_done == 0)
	{
		...
		// if ((new == NULL) && (char_read = 0) && (line[0] != '\0'))
		if (new == NULL) 
		{
			if (char_read == 0)
			{
...
/* ************************************************************************** */
// NEED to check "line_done": 0: endline NOT FOUND | 1: endline FOUND
...
char *create_segment(int fd, char *buf, int *line_done, ssize_t *char_read) 
{
	...
	if (end >= *char_read - 1)	// SI:		tous les caractère du buffer ont été analysé
